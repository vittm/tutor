...
var addressFind =[
  ...
  $('.distric-edit').append("<ul class='select-course'></ul>");
    $('#district').click(function(){
      ...
      _ul.html("");
      _ul.show();
      var city = $('#address-find').val();
      for (var i = 0; i < addressFind.length; i++) {
        if(addressFind[i].value ===  city){
          district = addressFind[i].cities;
        }
      }
      $.each(district, function(index, val) {
          _ul.append("<li>"+val+"</li>");
      });
      $('.distric-edit .select-course li').click(function(){
        _this.val($(this).text());
        $(this).parent().hide();
      });
    });
